...
st.markdown("An employee attrition predictor based on employee details designed for HR Managers")
st.divider()

Age = st.sidebar.slider('Age', 18, 70, 18)
...
TotalYears = st.sidebar.slider('Total Working Years', 0, 40, 10)
CurrRoleYears = st.sidebar.slider('Years in Current Role', 0, 20, 3)
PromoYears = st.sidebar.slider('Years Since Last Promotion', 0, 15, 1)
CurrMgrYears = st.sidebar.slider('Years with Current Manager', 0, 20, 3)
NumCompanies = st.sidebar.slider('Number of Companies Worked',0, 10, 2)
JobSatisfy = st.sidebar.slider('JobSatisfaction', 1, 4, 3)
EnvSatisfy = st.sidebar.slider('Environment Satisfaction', 1, 4, 3)
RelaSatisfy = st.sidebar.slider('Relationship Satisfaction', 1, 4, 3)
Balance = st.sidebar.slider('Work Life Balance', 1, 4, 3)
StockOptLevel = st.sidebar.slider('Stock Option Level', 0, 3, 0)
...
JobRole = st.sidebar.selectbox('Job Role', ['Sales Representative','Sales Executive', 'Research Scientist', 'Research Director', 'Manager', 'Manufacturing Director', 'Laboratory Technician', 'Human Resources', 'Healthcare Rep'])
